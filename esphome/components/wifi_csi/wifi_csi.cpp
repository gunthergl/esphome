...
extern esphome::wifi::WiFiComponent
...
float esphome::wifi_csi::CsiSensor::get_setup_priority() const { return esphome::setup_priority::AFTER_WIFI; }
...
void esphome::wifi_csi::CsiSensor::set_sensitivity(float sensitivity) { m_sensitivity = sensitivity; }
...
void esphome::wifi_csi::CsiSensor::update() {
  ...
  if (m_rssi) {
    ...
    if (difftime(now_t, last_t) > 5.0) {
      ESP_LOGD(TAG, "idx: %d, cnt: %d: avg: %.1f, current: %d, sensitvity: %d, motion: %d", idx, cnt, avgerageRssi,
               currentRssi, m_sensitivity, motion);
      last_t = now_t;
    }
  } else {
